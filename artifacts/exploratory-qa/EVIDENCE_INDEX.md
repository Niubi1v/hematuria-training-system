...
| HTML 报告 | 最后一次定向复现报告 | `reports/html/index.html` | 527,234 | 否 | 整个 HTML 报告原则上不入 Git | 证据根目录下同路径 |
| console/network/JSON/JUnit（18） | 首轮汇总与脱敏网络摘要 | `reports/*`（不含子目录） | 176,798 | 否 | 可由测试重建，避免报告噪声 | 证据根目录下同路径 |
...
当前本机保留 55 个自动生成证据文件、32,261,789 字节，另有本索引；已提交其中 3 个二进制证据文件、共 163,606 字节，并提交本索引。没有缓存、浏览器用户目录或临时依赖目录进入证据根目录。定向失败 trace 已关闭源码嵌入，仅保留页面快照和请求时间线。
