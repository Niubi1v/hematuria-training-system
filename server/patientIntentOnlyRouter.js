...
const rules = Object.freeze([
  ...
  ["smoking_history", (text) => has(text, /抽烟|吸烟|抽上|抽过烟|现在不抽|以前抽过|烟瘾|烟龄|烟这方面|烟这个东西|一天.{0,8}(?:烟|支|根)|(?:烟|支|根).{0,8}(?:一天|多少|几年|年)|戒烟|戒了没有|长期吸烟/)],
  ["medication_list", (text) => has(text, /(?:平时|平常|长期|常年|每天|固定|常规|规律|常用|常备|早晚|天天|一直备着).{0,14}(?:药|服用|服药|吃|用药)|(?:药|药物|服药|用药).{0,12}(?:平时|平常|长期|常年|每天|固定|常规|规律|常用|天天)|用药史|处方药|药物方面|列一张单子|慢性病.{0,8}吃什么药/)],
  ...
  ["prior_investigation_results_patient_aware", (text) => (has(text, resultWords) && has(text, /检查|化验|片子|报告|项目|尿一验|查完|查到|做完|外院|此前|以前|既往|那张|那份|最终/)) || has(text, /结果(?:咋样|怎么样|大概啥样)|(?:尿检|检查|化验|片子|报告).{0,8}怎么样|片子说啥|片子怎么讲|化验结论|查出什么|查到的是什么情况|医生.{0,8}检查.{0,8}有什么|项目结束后.{0,10}告诉|检查有没有说出|做完以后得到的说法|那堆结果|那个结果|片子怎么说|尿一验/)],
  ["prior_diagnosis_patient_aware", (text) => has(text, /诊断|病名|啥病|什么病|疾病名称|病因|怎么定性|认为是什么|什么问题|啥毛病|起过名|明确叫过什么|说您得的是啥|医生.{0,12}(?:下个说法|啥说法|说啥病|怎么说的|咋跟你讲|告诉过)|大夫.{0,12}(?:说法|讲)|外院.{0,12}(?:说法|结论)/) && !has(text, /化验|片子|报告|结果|尿一验/)],
  ["prior_investigations", (text) => has(text, /(?:以前|之前|此前|前头|先前|早先|来这前|来院前|外院|医院|过去|既往|别处|那次就诊).{0,20}(?:检查|化验|查过|查了|查什么|验过|抽血|拍片|片子|项目|检测|尿检|影像|检验)|(?:检查|化验|查过|验过|抽血|拍过?片|片子|项目|检测|尿检|影像|检验).{0,16}(?:以前|之前|此前|前头|先前|完成|哪些|什么|啥|哪几样|哪种|经历|包括|安排|做过|了吗|没|没有)|前面查了啥|做过项目没|验过哪些|片子拍了吗|那医院查了啥|医院给查哪些|去医院查什么|做过哪些项目|做过什么检查/) && !has(text, new RegExp(`${resultWords.source}|告诉|结束后|怎么讲|检查以后|查到`))],
  ["prior_medication_for_current_problem", (text) => has(text, /(?:这回|这次|此次|当前|为这事|为这个|为当前|为这症状|针对这次|针对刚才|症状起来后|尿红后|来前|来院前|来这里前|此次发作前后|前头|先前).{0,20}(?:吃药|服药|用药|用过.{0,5}药|用.{0,4}药|开药|给药|找药|药片|处方|吞过|服用|药物|打针|吃|服过)|(?:吃药|服药|用药|开药|给药|找药|药片|处方|吞过|服用|药物).{0,16}(?:这回|这次|此次|当前|为这事|为这个|症状|来前)|医院让你吃什么药|医生有没有.{0,8}开处方|医生开药|大夫给开药|当时.{0,6}给药|开了啥药/) && !has(text, responseWords)],
  ["treatment_response", (text) => has(text, /治疗反应|管用|管事|见效|效果|好转|好些|好点|缓解|松快|改善|维持|复发|反复|还犯|照旧|减少|恢复|有效|有没有用|还在吗|治后怎样|之后怎么样|以后怎么样|药物带来的变化|治疗.{0,8}怎么样|处理.{0,8}怎么样|吃药.{0,8}怎么样|用药.{0,8}怎么样|输液.{0,8}怎么样|(?:处理|治疗|吃药|用药|输液|办法).{0,12}(?:变化|区别)|症状变/) && !has(text, /用药史|有没有用药/)],
