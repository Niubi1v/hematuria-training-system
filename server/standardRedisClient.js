...
function redisNamespace() {
  const namespace = String(process.env.REDIS_KEY_PREFIX || process.env.REDIS_NAMESPACE || "").trim().toLowerCase();
  if (!/^[a-z0-9][a-z0-9:_-]{2,63}$/.test(namespace)) throw new Error("redis_namespace_invalid");
  return namespace;
}
...
function resolveStandardRedisConfig() {
  ...
  if (explicitUrl) {
    ...
    if (!host || !Number.isInteger(port) || port < 1 || port > 65535) return { configured: false, source: "none" };
  ...
  if (!["redis:", "rediss:"].includes(parsed.protocol) || !parsed.hostname) throw new Error("redis_url_invalid");
...
async function connectedClient() {
  ...
  if (!activeClient) {
    ...
    activeClient.on("error", () => {
      // Never log Redis URLs, credentials, commands, training tokens, or prompts.
    });
  }
