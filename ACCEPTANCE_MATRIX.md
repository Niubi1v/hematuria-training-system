...
基线：`codex/hematuria-production-goal`，起点`5a3ad11`；最新完整远程工程门禁HEAD `42cef081e8d2ea1c25de7d7742a46b3fbf56ce43`，已验收应用树`f5ab5539c10c6c8a62f99312dbdb37c722d845a9`。本文件按当前可重复证据校正状态；纯文档HEAD的CI证明仓库门禁，不替代父应用HEAD的真实交互证据。状态仅使用：
...
| 领域 | 强制标准 | 当前状态 | 当前证据 |
|---|---|---|---|
| API health | HTTP 200；仅非敏感配置状态/命名类型、版本和SHA；CORS受控 | PASS（Preview）/PENDING（Production） | `42cef08` Preview health HTTP 200并回报完整deployment SHA；训练签名与durable store保持configured且不显示值；正式Production health仍无当次证据 |
| 会话初始化 | 初始化不等待LLM；连续10次成功；P95≤3秒 | PASS（Preview）/PENDING（Production） | `f5ab553` Preview fresh session 10/10，端到端P95=1419ms、服务端session P95=12ms；Production仍待授权后验收 |
...
| PR CI | PR运行完整质量门禁且不部署Pages | PASS | HEAD `42cef08`的run `30356885044` completed/success；Node 22.14、Playwright 99/7/0、82页build、24资产bundle、secret和clean gate通过，Pages deploy按PR规则跳过 |
...
| DeepSeek真实中文5次、英文5次 | PASS（Preview）/PENDING（Production） | `f5ab553`受保护Preview：中文5/5、英文5/5均为`live_ai`、`deepseek-v4-flash`、thinking未执行，history-log 200；Production仍待授权后验收 |
...
| Azure四音色真实返回`audio/mpeg` | SKIP（未配置） | 当前未配置Azure；按目标约定明确SKIP，浏览器同语言语音降级合同已通过，不冒充云音频实测 |
...
| 强制标准 | 当前状态 | 当前证据 |
|---|---|---|
| TypeScript、ESLint、完整行为链 | PASS | run `30356885044`在Node 22.14下Unit/behavior、Typecheck、Lint均success |
| 69 JSON幂等、生成数据无漂移 | PASS | run `30356885044`验证转换幂等、提交基线及最终clean gate；`data/**`零差异 |
| 82页生产构建 | PASS | HEAD `42cef08` / run `30356885044`静态生成82/82 |
| 静态答案/密钥扫描 | PASS | run `30356885044`的repository与24资产bundle扫描success；文档变更后本地383个tracked/candidate文件扫描exit 0 |
...
| Playwright桌面/移动 | PASS | run `30356885044`在Node 22.14下99 passed、7个有依据的互斥skip、0 failed，用时8.6分钟；四视口与axe门禁包含于现有套件 |
...
| draft PR与GitHub Actions | PASS | PR #1保持Open/Draft；HEAD `42cef08`的run `30356885044` completed/success，Vercel Deployment与Preview Comments success，Pages deploy按PR策略skipped |
| Pages/Vercel SHA与live alias | PASS（Preview branch alias）/PENDING（正式live alias） | Preview health精确回报SHA `42cef081e8d2ea1c25de7d7742a46b3fbf56ce43`；公开Pages仍为旧main部署，正式Production alias未部署、未验证 |
...
当前工程专项、42例双语完整七阶段、HEAD `42cef08`的Draft PR Node 22 Actions，以及应用HEAD `f5ab553`的受保护Vercel Preview health、10次session、中文5次、英文5次、日志同步和P95均有真实证据；不存在仍待push或待CI的P0/P1工程候选。强制生产验收仍未完成，且以下项目不能由工程Agent自行关闭：

- `HEM-P0-001`、`HEM-P0-023`、419条模拟事实和42例`needs_revision`需要具名医学裁决与签署；
- Production 10+5+5、正式live alias及合并后Pages需要生产/合并权限；
- 真实手机软键盘、safe-area及患者自然度需要人工设备与教师验收；
- 正式教师鉴权、RCT数据库和正式OSCE还依赖approved病例、责任人及正式后端权限。

Azure当前未配置，按目标约定登记为`SKIP`；浏览器语音降级工程合同为PASS，不把未运行的云音频写成通过。
