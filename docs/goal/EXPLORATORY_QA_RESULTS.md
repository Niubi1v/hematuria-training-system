...
- 数据Agent结构审计新增HEM-P1-046，状态`FAIL_LOCAL_QA`：42例/257条结果的case/order归属、resultId唯一性、前置条件、非空结果和非终态显式内容均0结构失败；但13例的28/28条含数值final检验结果在结构化`unit`和`referenceRange`字段均为空。P001本地fixture使用真实结果行进入第2阶段后，生产报告卡稳定显示“单位—/参考范围—”，有效浏览器运行7/7失败；providerCalls=0，不替代Preview或真机。缺陷成立不需医学裁决，修复时实际单位/参考范围必须来自权威来源或具名医学审核。
- 数据Agent状态呈现新增HEM-P1-047，状态`FAIL_LOCAL_QA`：Production数据的257条结果使用`final=74`、`not_available=182`、`not_performed=1`三个内部状态值；QA用非医学fixture把三值送入Production报告卡，四固定viewport按中英文分配4/4复现三个token裸显（12/12观测），且带`abnormalLevel=positive`的`final`卡片4/4仍为`data-status=reported`。每次3个training-action均200，合计12/12，console warning/error为0；`providerCalls=0`，不替代Preview、真机或医学真值审核。
- 英文数据Agent新增HEM-P1-048，状态`FAIL_LOCAL_QA`：Production handler英文attempt全量审计连续2/2均为42/42病例、257/257结果、handler失败0，但42/42病例含学生可见CJK；57/60医嘱名、60/60主分类、49/60次分类、257/257结果分类/正文/印象和74个value含CJK，23个医嘱没有无CJK别名。P008英文`CBC`真实本地handler响应在四固定viewport 4/4显示44个中文控件标签且报告卡含中文；每个viewport handler与浏览器training-action均3/3为200，console warning/error为0，`providerCalls=0`。

## 2026-07-23 第 8 轮：Production `70ea9b3`

### 基线、第一阶段与 capability

- 远程 Production 精确为 `70ea9b3c7b31e11a84878de5c277cac60f35481c`。既有 QA 材料先以 `7899cd4` 普通 push，再以 merge `b94d7803507df3da52379f83ab05fef2afc45c87` 合入；未 reset/force push，业务实现与 `data/**` 无 QA 作者修改。
- Production 定向浏览器 12/12 通过：P001 中英文第一阶段、双向切换、刷新、AI capability 准备期间提交、P003 旧 token 替换与零轮提交、发送等待 capability、快速双击单请求、pending history 恢复。capability 服务矩阵连续两次均 19/19，`providerCalls=0`。
- 快速双击提交为 1 个 feedback request、1 个 request ID、1 个 timeline event；成功进入第二阶段并可刷新保持阶段。没有出现 `session_capability_required`、`invalid_attempt_token`、无限 `attempt_not_found`、`training_state_token_missing` 或重复事件。
- Preview 的 P003 零轮提交、P001 中文/英文、双向切换、刷新和快速双击均通过。核心正常流未出现意外 401/403/429/5xx；会话滥用批量运行末两项曾受 429 干扰，独立低频复跑 11/11 精确拒绝通过，不登记产品缺陷。

### Patient Agent 自然问法

- 新矩阵覆盖 42 例×10 问法×中英文，共 840 场景、1,428 个 intent 检查。canonical intent 为 1,134/1,428（79.41%），场景完整命中 630/840（75.00%）。
- 已知事实中错误 unknown 为 4/914（0.44%）；仅在 canonical 完整且无额外 slot/冲突的 439 个可评价事实中，极性错误 0/439。预期 unknown 为 436/436；42/42 医学冲突场景保持隔离，共覆盖 13 个唯一冲突项。
- 双语工程等义为 247/420，额外病史 slot 为 42/840。主要缺口集中于英文全程/分段、中文选择式血尿时相、英文终末否定、英文复合尿频尿急尿痛，以及英文“腰痛、发烧和血块”额外命中通用 pain。登记 HEM-P1-050；该结论只评价路由与披露边界，不审批医学值。

### 精确 Preview

| 指标 | 结果 |
| --- | --- |
| 10 次 session | 10/10；总耗时 P50/P95 `855/1364ms`，服务端 session `6/17ms` |
| 中文 live_ai | 5/5，fallback 0；回答 P50/P95 `1363/1443ms` |
| 英文 live_ai | 5/5，fallback 0；回答 P50/P95 `1349/1462ms` |
| 连续 20 轮 | 20/20 live_ai；20 agent、20 history、单 session、0 重初始化；回答 P95 `1383ms` |
| 后台恢复仿真 | 2/2 live_ai；2 agent、2 history、0 重初始化、无消息重复 |
| P038 双语五轮 | 10 agent/10 history/0 unauthorized；4 live_ai、2 safety boundary、4 rule fallback，source contract 2 组失败 |
| P037 刷新双语 | DOM恢复、8 agent/8 history、0 unauthorized；5 live_ai、2 safety boundary、1 rule fallback，英文时长连续性失败 |

- 英文错误总结与无指代澄清在全套和隔离复跑均被 `rule_fallback` 接管；P038/P037 独立复跑排除了 history 超时和 401，稳定问题是 source contract。合并登记 HEM-P1-051。安全边界不冒充 live_ai；当前非流式接口的 server `firsttoken` timing 不冒充真实流式 TTFT。

### 42 例、七阶段、UI 与交付判断

- 本地目录点击、直接 URL、刷新、中英文为 42/42；42 个壳页均显示七阶段且提交前无标准答案/评分点泄露。精确 Preview 同为 42/42，P999 受控 404。GitHub Pages 未取得当前 SHA 独立部署证据，保持部署阻塞。
- P001 七阶段在 `1440×900` 与 `390×844` 2/2；7 个 stage feedback、7 个唯一 request ID、exam/order/score 各一次，最终报告按 360 分显示，本次 fixture 得分 220/360，重复计分 0。
- HEM-P1-027 开场双语×四视口 8/8、手动上翻/新消息入口 8/8，Production composer 几何 4/4，均为 `PASS_EMULATION`；真机软键盘与 safe-area 仍 `BLOCKED_REAL_DEVICE`。
- HEM-P1-046/047/048 在当前 SHA 的 `1440×900` 各 1/1 稳定失败，保持 OPEN；没有把旧缺陷当作本轮新增。新增缺陷为 P1 两项（HEM-P1-050/051），新增 P0/P2 为 0。
- 教师人工审阅建议：可在工程修复后进入受限的语言/教学体验审阅；当前不建议做最终教师验收，因为 HEM-P1-050/051 与数据 Agent P1 尚开放，HEM-P0-001/023 仍需具名医学裁决。

### `70ea9b3` 基线切换前的长期回归补录

- 全 42 例双语七阶段 UI 旅程补充执行：42 例×中英文共 84 条完整旅程、588 次阶段提交、84 份 360 分报告全部通过；同套件另完成 1 条移动 viewport 代表旅程、7 次阶段提交和 1 份评分报告。浏览器工程证据标记 `PASS_EMULATION`，不替代真机。
- Production `tests/e2e/practice.spec.mjs` 完整回归为 72/72，通过第一阶段、capability 等待、双击防重、切换/刷新、20 轮、history 恢复、离线重连、fallback→live_ai fixture 恢复、阶段锁、语音降级、可访问性、P008 开单与评分抗伪造。
- 10 个确定性服务端合同脚本全部通过，覆盖阶段 5/6/7、42 例双语阶段流、360 分事件评分与对抗评分、attempt 隔离、训练 API/安全、API/AI 恢复和 Agent API 安全。capability 公共 handler 矩阵另连续 2 次 19/19，通过项共 38、失败 0、`providerCalls=0`。
- 上述补录没有运行真实 Preview provider、没有裁决医学事实，也没有使用真机；`BLOCKED_MEDICAL`、`BLOCKED_SOURCE_REVISION` 与 `BLOCKED_REAL_DEVICE` 保持不变。两份重复 capability 原始 JSON 只在本机保留，Git 仅保存脱敏聚合摘要。

## 2026-07-23 第 9 轮：Production `c4ac9b5`

### 基线与合并边界

- `git fetch --prune origin` 后远程 Production 精确为 `c4ac9b5a59021bed10dc2d94c4ebf4d8f97badd2`。现有 QA 补录先以 `6cdcfbb33e86fde269f0f800614a5505210eadce` 普通 push，再以 merge `706a758` 纳入新基线；未 reset、rebase 或 force push。
- 三方预检和实际 merge 的唯一冲突为 `tests/preview/preview-blackbox.spec.mjs`。解决结果保留QA精确aria状态断言并纳入Production新live_ai helper；`app/src/api/server/data`无冲突，合并后业务路径相对Production差异为0。

### HEM-P1-050 与 HEM-P1-051

- HEM-P1-050 为 `RESOLVED_LOCAL_QA`：自然问法840/840、canonical checks 1,428/1,428、大矩阵3,150/3,150；错误unknown 0/914、极性错误0/598、正确unknown 436/436、冲突隔离42/42、双语等义420/420、额外病史0、providerCalls=0。指定的否定、选择、复合、dysuria与generic pain边界全部覆盖。
- 精确Preview health回报完整SHA `c4ac9b5a59021bed10dc2d94c4ebf4d8f97badd2`。核心9/9；P001英文纠错/澄清3/3、P037上下文2/2、P038上下文2/2均`live_ai/deepseek/non-fallback`，history 7/7。HEM-P1-051 标为 `RESOLVED_PREVIEW`，没有被rule fallback接管。
- Preview session 10/10，总耗时P50/P95 1132/1871ms、服务端session 6/14ms；中文live_ai 5/5、回答P50/P95 1292/1315ms，英文5/5、1423/1971ms，fallback均0。单session连续20/20 live_ai、20 agent/20 history、0重初始化、刷新消息保持，回答P95 1630ms。
- Chromium `frozen→active` 后两轮均live_ai，agent/history 2/2、attempt/session重初始化0、消息无重复；标记为浏览器生命周期仿真，不替代真机后台。真实受控provider超时/429、history 503和Redis不可用故障注入未获授权，保持 `BLOCKED_PREVIEW_FAULT_INJECTION`。

### Data Agent 与新增 HEM-P1-052

- Production呈现合同通过：60医嘱、257结果、16查体项；28项缺unit/referenceRange均显示等待审核元数据，23个缺英文审核名称显示占位并在UI禁用，英文API CJK=0。desktop/mobile三项呈现用例6/6，异常状态优先于final。
- 新增HEM-P1-052，`FAIL`：合法英文attempt中，23/23个缺审核英文名称的医嘱仍可用内部ID被公开handler匹配，6项返回确定性结果；其中4个医嘱关联29条病例评分规则，29/29均形成earned事件，共641分。独立进程2/2完全一致。页面禁用不能替代服务端评分隔离；QA未修改业务代码或医学数据。
- 28项真实单位/参考范围继续 `BLOCKED_MEDICAL`，23个英文名称与161个来源修订继续 `BLOCKED_SOURCE_REVISION`；工程安全文案通过不等于医学/来源通过。

### 完整回归与交付判断

- 本地practice desktop/mobile 78/78；全42例双语七阶段为2个实际项目通过、2个互斥项目按设计skip，覆盖84旅程、588阶段提交、84份360分报告及1条mobile代表旅程。14个阶段/评分/attempt/session/恢复/Patient安全合同14/14。
- Preview路由：目录中英文42/42、直接URL42/42、刷新42/42、P999受控404。root与Pages basePath本地静态构建均82/82；Pages仿真有效病例42/42、P999 404、25个JS bundle扫描通过。真实GitHub Pages部署未由本地仿真替代。
- 本轮新增P0/P1/P2为0/1/0。由于HEM-P1-052开放，不建议进入最终教师人工审阅；可在其修复后进入受限语言/教学体验审阅。HEM-P0-001/023与P019/P020审核状态保持不变，真实手机继续 `BLOCKED_REAL_DEVICE`。

## 2026-07-24 第 10 轮：`c4ac9b5` 开放缺陷回归

- HEM-P1-045更新为`RESOLVED_PREVIEW`：P037中文/英文各4轮，共8/8 DeepSeek live_ai、8 agent/8 history；刷新前后DOM 6→6，刷新后401=0、缺history=0、attempt/session重初始化=0，病程连续性2/2，语言/教师/结构/诊断泄露均0。
- HEM-P1-030继续`REGRESSED_LOCAL_QA`：42例×37槽位矩阵连续2/2均为6,216路由、6,216重放、42个失败实例/1组；英文泌尿操作史问法42/42实际命中`triggers`而非`PAST_URINARY_PROCEDURE`。新增公开handler最小探针连续2/2同样失败，每次18项检查仅该1项失败，providerCalls=0。
- HEM-P2-028继续`RESOLVED_LOCAL_QA`：桌面重复批6/6，以及`1440×900`/`390×844`代表批2/2，单次动作均由断言证明为1个feedback request、1个request ID、1条timeline事件。历史失败截图/trace保留，不用通过证据覆盖。
- HEM-P2-044仍为`FAIL_EMULATION`：`390×844`和`360×800` 2/2复现入口106×38、关闭26×28、试听75×38、停止34×38；桌面键盘、焦点、Escape和reduced-motion 2/2通过，真机仍`BLOCKED_REAL_DEVICE`。
- HEM-P1-052扩展复现2/2：英文23/23内部ID匹配、23/23响应仍自报translation unavailable、6项返回结果、29/29评分链得641分；同一23项中文路径23/23匹配，在相同前置状态返回6项结果。结论限定为语言特异服务端隔离缺失，不能建议全局禁用。
- 本续轮新增P0/P1/P2为0/0/0；开放工程项仍为HEM-P1-030、HEM-P1-052和HEM-P2-044。QA运行器新增可选重复次数并对Next日志排空设置5秒上限，修正了测试完成后外层超时；后续隔离运行已正常返回。

## 2026-07-24 第 11 轮：`c4ac9b5` P001–P042 真实 Preview 主诉抽样

- 通过安全 Automation Bypass wrapper 对此前未逐例抽样的 P006–P012 执行三轮双语开放式主诉，共 42 个自然问题操作；中文 21/21 为 DeepSeek `live_ai`，英文 21/21 为 `rule_fallback`，公开原因均为 `classifier_disabled`。三轮均逐例一致，登记 HEM-P1-053。
- 为区分“自然问法未命中”与provider失败，第三轮在同一7个英文session追加已由本地canonical matcher识别的开放式主诉控制问法。7/7均实际进入DeepSeek，但最终为`safety_boundary`；安全debug布尔值为`responseAccepted=false`、`rewriteTriggered=true`，说明首答及重写均未通过患者输出过滤。公开fallback原因是泛化的`ai_unavailable_or_rule_mode`，不能据此把provider调用误写成不可用。
- 三次运行共49个操作，agent request 49/49、history-log 49/49，HTTP失败、重复request/history、语言串线、教师元语言、结构化字段泄露和跨origin保护头请求均为0。完整问题与回答、request body、session/token/header和任何凭据均未保存；Playwright原始输出在实际凭据字节和敏感header扫描通过后删除。
- 随后以完全相同断言补齐P001–P005和P013–P042。全42例诊断批为中文自然问法42/42 live_ai、英文自然问法42/42 classifier-disabled rule fallback；英文canonical控制42次均进入DeepSeek，其中31/42 live_ai，P001–P003及P005–P012共11/42为重写后仍被拒绝的safety boundary。
- 连同P006–P012前两次自然问法复跑，本轮累计154个合法操作：自然中文56/56 live_ai、自然英文56/56规则fallback、canonical英文31/42 live_ai；agent/history154/154，HTTP、倍增、401/403/429/5xx及泄露失败均0。一次沙箱网络拒绝发生在到达应用前，经授权重跑后通过，不计产品结果。
- 结论扩展为P001–P042英文自然开放主诉的全量工程路由失败；输出过滤子路径精确限定为P001–P003、P005–P012，不外推其余31例。该结论不裁决患者回答中的医学内容。本轮新增P0/P1/P2仍为0/1/0；开放工程项为HEM-P1-030、HEM-P1-052、HEM-P1-053和HEM-P2-044。
- 该P1不阻止后续病例和中文路径继续独立QA，但在HEM-P1-052/053修复前不建议进入最终教师人工审阅。HEM-P0-001/023、28项医学元数据、23个英文审核名称、161个来源修订与真实手机仍保持原阻塞状态，未被本轮工程结果覆盖。

## 2026-07-24 第 12 轮：`c4ac9b5` P003 真实 Preview 七阶段与终末幂等

- P003中文零轮第一阶段后完成真实Preview七阶段共3/3；每次7个合法`stage-feedback`均200并带request ID，随后恰有1个`score` 200，360最终报告3/3可见。两次带刷新诊断均保持最终报告，刷新没有新增stage-feedback或score。
- HEM-P2-028在阶段7真实Preview回归：同步双击“完成训练并生成最终报告”3/3均发出两个独立debrief请求（200与409 `stage_not_unlocked`）和一个score请求；最新带UI诊断的2/2运行同时错误显示“终末评分服务暂时不可用”，尽管score 200且报告已生成。阶段1–6的本地`1 request / 1 request ID / 1 timeline event`结论不被改写，但不能代替阶段7 Preview。
- 刷新后的`validate-attempt`为401 `attempt_already_completed`，页面仍以已完成状态恢复最终报告，故按受控终态记录，不计恢复失败。最小复现保留强断言，未删除或放宽失败条件。
- session初始化在4次诊断迭代中出现2次200与2次409 `stale_attempt_token`，最近连续2次均200；当前证据不足以独立登记稳定缺陷，只作为间歇性观察保留，后续定向复测。
- 本轮新增P0/P1/P2为0/0/0；既有HEM-P2-028从`RESOLVED_LOCAL_QA`更新为`OPEN / FAIL_PREVIEW_STAGE_7`。测试未询问医学事实，未保存回答、request body、request ID或凭据。

## 2026-07-24 第 13 轮：`c4ac9b5` 逐病例复合详细病史

- 新增42例×中英双语×10组复合病史矩阵，共786个场景、786次重复。两次独立进程输出逐字节一致；providerCalls=0、重复结果786/786稳定。问句覆盖血尿可见性/时相/颜色/血块/疼痛、尿路症状与急症、肾小球线索、既往结石/感染/肿瘤、高血压/糖尿病、用药/过敏、吸烟/职业/家族、手术/输血/外伤/泌尿操作及女性月经/妊娠。
- 发现HEM-P1-054：786个场景中689个不满足完整槽位合同；618个canonical+structured跨层场景有613个失败。按实例计为canonical自然alias遗漏84、structured matcher遗漏126、被静默丢弃子句657、错误附加槽位74、既往肿瘤史被误判诊断边界42。两个纯canonical复合对照也有76/168失败，主要是长回答被整体安全阻断或英文alias缺口。
- 医学治理没有被当作失败：56个含HEM-P0-023冲突槽位的场景56/56保持`pending_medical_review`隔离；QA不比较病例事实值，也不解除HEM-P0-001/023。
- 精确Preview以P001–P007中文、5组低频复合问句验证35个操作。降速正式运行35/35 agent与35/35 history均200、一一对应，跨origin保护头泄露0；但35/35至少遗漏一个已询问结构化病史子句，7/7“发热/感染/既往肿瘤”问句误入`diagnosis_boundary`。来源为7 live_ai、13 rule_fallback、15 safety_boundary；无通用unknown和教师/结构/语言泄露。
- 首次Preview诊断批第35个操作触发429并由客户端重试，故该轮只用于识别速率噪声；增加1.5秒间隔后的正式运行无401/403/429/5xx，不把限流样本计入HEM-P1-054。
- 本轮新增P0/P1/P2为0/1/0。开放工程项增加HEM-P1-054；在HEM-P1-030/052/053/054修复前仍不建议进入最终教师人工审阅。

## 2026-07-24 第 14 轮：`c4ac9b5` Data Agent阶段权限与前置条件恢复

- 新增42例×中英双语的生产`training-action`黑盒矩阵：84个合法attempt、336个阶段权限探针、252次阶段状态验证、1,344次查体选择、84个批量开单和392条允许释放结果检查。阶段1过早调用、阶段3过晚调用均按合同拒绝；752个已配置查体、592个未配置查体、全部中文257条结果及有审核英文来源的135条结果均精确映射，跨病例/教师字段/CJK泄露和其他失败为0。
- 28条单位/参考范围缺口继续`BLOCKED_MEDICAL`；23个未审核英文目录名、122条对应结果和376条英文查体来源继续`BLOCKED_SOURCE_REVISION`。英文未审核项目被排除于“工程可见性通过”计数，没有将安全占位文案写成来源完成。
- 既有相关回归继续通过：Data Agent呈现60医嘱/257结果且英文API CJK信号0、`dataChanged=false`；查体质控42例/376条；P008精确医嘱与前置条件静态合同。该静态P008合同没有覆盖“先失败后补齐再重试”，不能替代HEM-P1-055。
