...
## 2026-07-19 完成度审计与静态404补证

| 命令/检查 | 退出码 | 结果 |
|---|---:|---|
| `git fetch --prune origin`及`git rev-list --left-right --count HEAD...origin/codex/hematuria-production-goal` | 0 | 恢复基线`3a16f93`，ahead/behind `0/0`，工作树干净 |
| root静态服务：`serve-static.mjs` + HTTP探针 | 0 | `/`、P001、P013、P042均200；P999为404 |
| `BASE_PATH=hematuria-training-system`静态服务 + HTTP探针 | 0 | basePath根、P001、P013、P042均200；P999为404；basePath外P001为404 |
| 静态服务清理 | 0 | 两个隐藏测试进程均在`finally`停止，无端口/进程残留作为通过前提 |
| Goal/Acceptance交叉审计 | 0 | 将旧候选SHA、待push及待Node22状态校正到`3a16f93`/run `29547532678`；Production、Pages正式发布、真机与医学人工项仍保持PENDING/HUMAN/BLOCKED |

本轮没有重新执行已在`3a16f93`远程通过且应用树未变化的昂贵完整门禁；修改仅为权威文档和验收证据。提交前diff检查、323-file repository secret scan和`data/**`零差异检查均已通过。

## 2026-07-20 教师验收整改本地证据

本地运行时为bundled Node `24.14.0`；仓库强制Node22.14，故Node22结论必须由新HEAD Actions补证。

| 命令/合同 | 退出码 | 结果 |
|---|---:|---|
| `tsx scripts/test-patient-priority-paraphrases.ts` | 0 | 42×24，1008/1008 |
| `tsx scripts/test-patient-intent-normalization.ts` | 0 | 86/86；错误unknown 0；极性错误0；复合完整 |
| `tsx scripts/test-patient-paraphrase-matrix.ts` | 0 | 3150/3150；known 1370；correct unknown 1715；冲突隔离65；极性错误0 |
| `tsx scripts/test-patient-semantic-classifier.ts` | 0 | 白名单/strict JSON/0.92阈值/缓存/singleflight/限流/provider失败/canonical投影/端到端语义分类均通过 |
| `tsx scripts/test-patient-prompt-audit.ts` | 0 | 本地限定、Production/Vercel强制关闭、随机canary递归脱敏通过 |
| `tsx scripts/test-teacher-cors-contract.ts` | 0 | Pages精确origin与同源Preview 204；X-Request-Id等4头允许；恶意origin 403；无wildcard |
| 完整`package.json#test`命令链 | 0 | 37.3秒；完整行为、安全、572、153/419、workbook、attempt/session、Patient、360均通过。沙箱内首次因xlsx junction不可见中断，同命令只读允许访问依赖后通过 |
| `tsc --noEmit` | 0 | 沙箱外只读运行；无TypeScript错误 |
| `node scripts/run-lint.mjs` | 0 | ESLint 0 error/0 warning |
| Playwright自管Next | 124 | 72 passed/2 skip/0 failed，但Windows子进程10分钟未退出；不作为门禁通过 |
| `PLAYWRIGHT_EXTERNAL_SERVER=1 playwright test`（受控Next job） | 0 | 72 passed/2互斥skip/0 failed，209.6秒；desktop/mobile及42例双语完整流程 |
| Vercel等价：`VERCEL=1 VERCEL_ENV=preview next build` | 0 | 82/82，16.6秒；bundle 25 JS通过 |
| Pages：`NEXT_PUBLIC_BASE_PATH=/hematuria-training-system NEXT_PUBLIC_API_BASE_URL=https://hematuria-training-system.vercel.app next build` | 0 | 82/82，18.4秒；basePath bundle 25 JS通过 |
| `node scripts/scan-repository-secrets.mjs` | 0 | 336个tracked/candidate + reachable history；无值输出 |
| `git diff --exit-code -- data` | 0 | `data/**`零差异；医学事实/审核状态未改 |

外部证据边界：公开Pages deployment已确认`main@5a3ad119`；当前外部Vercel OPTIONS连接超时，未取得响应头。新HEAD尚未push，故Actions、Vercel Ready SHA、真实Preview第一阶段提交均为PENDING，不能以本地结果替代。

## 2026-07-20 教师验收整改CI生成基线恢复

| 命令/检查 | 退出码 | 结果 |
|---|---:|---|
| Actions run `29712230950` / HEAD `4ff2d04` | 1 | Node 22.14；首个真实失败为Conversion idempotency，3个英文生成物基线漂移；后续门禁skipped |
| `tsx scripts/test-chief-complaint.ts` | 0 | 运行时自然文案与稳定生成合同均通过，含P019/P020待复核主诉保护 |
| `tsx scripts/test-conversion-idempotency.ts` | 0 | 提交后在隔离worktree验证75个受控输出首轮基线和第二轮幂等均通过 |
| `tsx scripts/test-patient-facing-profile.ts` | 0 | 42例患者可见资料完整性通过 |
| `tsx scripts/test-public-routes.ts` | 0 | 42例local/Vercel root及Pages basePath路由合同通过 |
| `tsc --noEmit` / `node scripts/run-lint.mjs` | 0 / 0 | TypeScript与ESLint通过；TypeScript需沙箱外只读xlsx依赖访问 |
| Vercel-equivalent `next build` | 0 | 82/82静态页；25个JS资产bundle scan通过 |
| Pages basePath `next build` | 0 | 82/82静态页；25个JS资产bundle scan通过 |
| `node scripts/test-secret-scanner.mjs` / `scan-repository-secrets.mjs` | 0 / 0 | 工作簿/历史合同及336个tracked/candidate/历史扫描通过，无值输出 |
| `git diff --exit-code -- data` | 0 | `data/**`零差异；未接受诊断时产生的生成物漂移 |

本轮只改生成器职责边界及其测试；此前应用行为、72/2 Playwright及完整行为门禁未受该代码路径影响，没有重复运行昂贵全量套件。Node 22远程通过仍须以修复提交push后的新run为准。

## 2026-07-20 真实Preview能力会话竞态

| 命令/检查 | 退出码 | 结果 |
|---|---:|---|
| Actions run `29712640050` / HEAD `363aa17` | 0 | Node 22.14；Playwright 72 passed/2 skipped，Conversion/行为/医学/安全/82页/bundle/clean gate全绿；Pages deploy按Draft规则skipped |
| `node scripts/run-preview-blackbox.mjs` / HEAD `363aa17` | 1 | health与P003零轮提交通过；P001中文首问`agent-chat`为401 `session_capability_required`；5 passed、1 failed、2未运行；输出扫描2文件通过 |
| 同次Preview稳定性 | 部分通过 | session 10/10，P95 1743ms；中文live AI/history-log 5/5，P95 1497ms；英文5/5，P95 1264ms；不能替代失败UI流程 |
| 新浏览器延迟session测试 | 0 | capability前按钮禁用、Enter不发请求；capability后1个合法agent-chat；desktop/mobile均通过 |
| 完整本地Playwright | 0 | 74 passed/2互斥skipped/0 failed，3.1分钟；含42例双语完整UI阶段矩阵 |
| `tsc --noEmit` / ESLint / Preview安全canary | 0 | 类型、lint、10错误路径与5 artifact通道、Preview config均通过 |
| Vercel/Pages两种build与bundle | 0 | 各82/82页面，各25个JS资产扫描通过 |
| repository/工作簿secret scan | 0 | 336 tracked/candidate及历史、压缩工作簿合同通过，无值输出 |
| `git diff --exit-code -- data` | 0 | 医学数据与审核状态零差异 |

黑盒证据仅记录`sessionCapabilityPresent`布尔值，绝不记录session值、training token、Cookie、Authorization、签名或Automation Bypass凭据。新代码尚未push，本节不宣称Preview竞态已线上修复。

## 2026-07-20 HEAD 296bf7e 能力会话竞态远程验收

| 命令/检查 | 退出码 | 结果 |
|---|---:|---|
| `git fetch --prune origin`及HEAD对账 | 0 | 本地/远程均为`296bf7e6f2e797c634c762b67488b279dfe59a37`，ahead/behind `0/0`，工作树干净 |
| Actions run `29719580921` | 0 | Node 22 job completed/success；行为/医学/安全、TypeScript、ESLint、secret scan、Playwright、82页build、bundle与clean gate通过；Pages artifact skipped |
| Vercel checks | 0 | Deployment success；Preview Comments success；部署health返回精确SHA `296bf7e6f2e797c634c762b67488b279dfe59a37` |
| `pnpm run test:e2e:preview` | 0 | 8 passed，1.3分钟；输出凭据扫描1个生成文件通过 |
| P003零轮中文 | 0 | `init-attempt=200`、`stage-feedback=200`，进入第二阶段；随后的晚到`session/init=409 stale_attempt_token`未覆盖成功提交 |
| P001中文首问/刷新/双击 | 0 | `session/init=200`、`agent-chat=200`、`sessionCapabilityPresent=true`、DeepSeek `live_ai`、`history-log=200`、`validate-attempt=200`、单次`stage-feedback=200` |
| P001英文首问 | 0 | `agent-chat=200`、DeepSeek `live_ai`、`history-log=200`、`stage-feedback=200`；云TTS 403按浏览器语音降级，不阻断问诊 |
| P001中英双向切换 | 0 | zh→en→zh均重新取得对应attempt；最终session初始化及阶段提交200，无旧token复用 |
| Preview session稳定性 | 0 | 10/10；端到端P95 1699ms、服务端session P95 9ms |
| Preview中文live AI | 0 | 5/5；回答P95 1534ms、provider P95 1139ms、首Token P95 864ms、history P95 7ms |
| Preview英文live AI | 0 | 5/5；回答P95 1327ms、provider P95 933ms、首Token P95 727ms、history P95 6ms |

保护头只注入目标Preview origin，跨origin为0；证据不含bypass值、Cookie、Authorization、session/attempt token或完整签名。该远程证据关闭HEM-P1-049，不代表Production正式部署、真实Azure TTS或医学专家裁决通过。

## QA c83c7d5 P1整改本地证据（2026-07-23）

本地使用bundled Node `24.14.0`；仓库强制Node 22.14，因此Node 22结果须由push后新HEAD Actions确认。

| 命令/合同 | 退出码 | 结果 |
|---|---:|---|
| `tsx scripts/test-patient-priority-paraphrases.ts` | 0 | QA自然840/840；canonical checks 1428/1428；总1848/1848 |
| `tsx scripts/test-patient-paraphrase-matrix.ts` | 0 | 3150/3150；known 1370；correct unknown 1715；错误unknown 0；极性错误0；隔离65 |
| `tsx scripts/test-patient-pain-routing.ts` | 0 | 42×6特异疼痛；5例冲突隔离；generic pain无额外扩张 |
| `tsx scripts/test-bilingual-patient.ts` | 0 | 42例×6英文复合问法；完整门禁首次发现独立pain过窄过滤，`86f5ad9`修复后通过 |
| `tsx scripts/test-patient-contextual-followup.ts` | 0 | P001纠错/澄清、P037/P038多轮、live来源保持、401 fallback及恢复、report detail阻断 |
| `tsx scripts/test-data-agent-presentation.ts` | 0 | 60医嘱、257结果、23缺英文别名fail-closed、28缺元数据fail-closed、16查体项、API英文CJK=0、dataChanged=false |
| Data Agent browser专项，desktop/mobile | 0 | 6/6：英文控件/报告0 CJK；缺元数据显示2处待审核；异常优先及状态本地化 |
| `pnpm run test` | 0 | 完整行为、安全、Patient、42例、572、153/419、workbook、attempt/session、360全部通过 |
| `PLAYWRIGHT_EXTERNAL_SERVER=1 playwright test` | 0 | 80 passed、2个项目互斥skip、0 failed，213.1秒；含桌面42×双语七阶段及移动代表旅程 |
| `tsc --noEmit` / `node scripts/run-lint.mjs` | 0 / 0 | TypeScript和ESLint通过；沙箱内首次因xlsx junction不可见失败，沙箱外同锁文件只读依赖通过 |
| Vercel同源：`VERCEL=1 VERCEL_ENV=preview pnpm run build` | 0 | 82/82静态页；25个JS资产bundle scan通过 |
| Pages：`NEXT_PUBLIC_BASE_PATH=/hematuria-training-system NEXT_PUBLIC_API_BASE_URL=https://hematuria-training-system.vercel.app pnpm run build:github` | 0 | 82/82静态页；basePath bundle 25 JS通过 |
| `node scripts/scan-repository-secrets.mjs` | 0 | 340个tracked/candidate加可达历史及有界二进制/归档元数据，无敏感值 |
| `git diff --quiet -- data` | 0 | `data/**`零差异；医学事实、reviewerStatus、needs_revision及360规则未改 |

说明：Playwright直接管理Next时，本机Node 24在断言结束后无法回收开发服务器，外层命令超时；同一测试以受控外部Next进程执行正常exit 0。该基础设施现象没有被写成产品失败，也没有用retry或放宽timeout掩盖断言。真实Preview、Node 22、Actions与Vercel结果仍为PENDING。

## 依赖审计CI恢复本地证据（2026-07-23）

本地bundled Node为`24.14.0`、pnpm为`11.9.0`；仓库权威Node 22.14结果须由新HEAD Actions确认。

| 命令/合同 | 退出码 | 结果 |
|---|---:|---|
| `pnpm audit --audit-level high` | 0 | 0 high/critical，仅余1 moderate；Next 15.5.21、Sharp 0.35.0、brace-expansion 1.1.16/5.0.7 |
| `pnpm run typecheck` / `pnpm run lint` | 0 / 0 | 3.3秒 / 3.9秒 |
| `pnpm run test` | 0 | 36.3秒；Patient、Data Agent、42例、572、153/419、18冲突隔离、session/attempt及360门禁通过 |
| `PLAYWRIGHT_EXTERNAL_SERVER=1 playwright test` | 0 | 80 passed、2个项目互斥skip、0 failed，187.4秒；runner正常退出 |
| `VERCEL=1 VERCEL_ENV=preview pnpm run build` + `test:bundle` | 0 | 82/82页；25个JS资产通过 |
| Pages basePath `pnpm run build:github` + `test:bundle` | 0 | 82/82页；25个JS资产通过 |
| `pnpm run test:secrets` | 0 | 340个tracked/candidate及可达历史和有界归档元数据，无敏感值输出 |
| `pnpm run test:idempotency` | 0 | 在提交`6c1d42c`上验证75个受控输出，首轮与第二轮一致 |
| `git diff --exit-code -- data` | 0 | 医学数据零变化 |

Actions run `30008877764`只证明旧HEAD `141f5bb`的依赖审计失败；不能替代`6c1d42c`之后新HEAD的Node 22、Playwright、Vercel或Preview结果。

## QA P1整改远程证据（2026-07-23）

| 远程门禁 | 结果 |
|---|---|
| GitHub Actions `30011651645` / `c9f7807` | success，Node 22.14.0，build 11分21秒 |
| Full dependency audit | success；1 moderate，0 high/critical |
| Conversion idempotency / data baseline | 75个受控输出通过；`data/**` clean |
| Unit/behavior/medical/security | success；含840/1428/3150、42例、572、153/419、18冲突、360评分 |
| Playwright E2E | 80 passed、2 skipped、0 failed，8.4分钟 |
| Static build / bundle / clean gate | 82/82页；23个JS资产；clean gate success |
| Vercel Deployment / Preview Comments | success / success；health SHA `c9f7807` |
| Preview基础黑盒 | 8/8；P003零轮、P001中英文、切换、刷新、双击、第二阶段、10+5+5 |
| Preview扩展黑盒 | 11/11；新增P001英文纠错/澄清、P037/P038多轮来源门禁 |
