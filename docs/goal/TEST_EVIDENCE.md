...
Preview本轮未取得登录态请求证据：in-app Browser与Chrome控制客户端均在初始化失败，匿名分支请求超时；不得填造病例session、HTTP状态或console。既有环境名称/作用域审计与`server/trainingAttemptStore.js`只支持“缺Preview共享store时会503 fail-closed”的证据链，当前Preview实际失败请求仍需登录态人工复测确认。
