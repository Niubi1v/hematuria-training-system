...
## 2026-07-26 第 27 轮 `9b7fcd0` HEM-P1-030关闭复测

- 用42例×37槽位×双语×双问法矩阵连续双跑，并对缺陷原始英文泌尿操作史问法单列治理路由、可收集路由、受控unknown和`triggers`泄漏计数。
- 公开adapter同步按治理层/可收集层双合同双跑；不可收集事实必须内部精确命中、公开slots/facts为空且返回允许的受控原因，不得把审核阻塞当路由失败或工程通过。
- 相邻门禁保持病史路由、安全投影和18项双语冲突隔离；本地rule合同不触发真实Preview、全量Playwright或昂贵七阶段回归。
- HEM-P1-030关闭后继续从尚未独立关闭的工程缺陷中选择下一范围；HEM-P1-057/058、医学/来源修订、真机和外部权限阻塞不变。

## 2026-07-26 第 28 轮 `9b7fcd0` HEM-P1-050自然问法复测

- 双跑42例×10类×双语自然问法与1,428 intent检查；另双跑15 intent×5问法的3,150矩阵和1,848优先改写门禁。
