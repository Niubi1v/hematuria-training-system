# 自然语言与双语审查

状态：真实 Preview 已完成当前 SHA 的中英文来源/稳定性最低样本；逐答复定性、错误总结纠正与医学语义审查仍持续，医学冲突不由 QA 裁决。

## 审查维度

- 语言纯度：中文不夹杂无关英文，英文不回落中文。
- 对问性：回答只覆盖所问事实，不倾倒完整病例摘要。
- 一致性：同一事实不同问法保持一致；错误总结时患者应明确纠正。
- 自然度：避免机械重复、列表式答案、过多专业术语和教师口吻。
- 安全性：不泄露诊断、隐藏病史、评分点、标准答案或未经提问的关键事实。
- 来源真实性：`fixture`、`rule_fallback`、`live_ai` 分开统计。

## 当前结论

- `HEM-P0-023` 的 18 条冲突只记录为待专家裁决，不做医学真值判断。
- 首轮 20 轮脚本用于 UI 稳定性和问法覆盖，回答为明确标记的脱敏 fixture，不能用于自然度或医学一致性通过结论。
- 历史首轮未取得真实样本；当前 `3a16f931` 的真实 Preview 增量结论见文末，不能用后续工程证据改写当时历史事实。

## 首轮样本

- 已运行 20 个中文问法，包含起始/反复/颜色/肉眼镜下/时相/血块/疼痛/LUTS/急症/肾小球线索/既往史/用药过敏/暴露/家族史/女性来源/操作外伤，以及重复起始时间和故意错误总结。
- 加上快速双击测试共保存 21 轮脱敏 transcript；患者答复为序号化固定 fixture，只用于 UI、日志幂等和恢复，不评价自然度或医学正确性。
- 本轮未取得真实英文对话样本；病例目录英文视觉通过不等于英文 Patient Agent 通过。

## 2026-07-14 fixture 协议审查

- `test-bilingual-patient.ts` 对 42 例各 6 个英文 fixture 问法通过，覆盖语言保持、报告/诊断边界和冲突隔离；P001 浏览器 fixture 另验证英文会话初始化、英文答复显示及中英文 attempt 隔离。
- `test-structured-history-matrix.ts` 对 42 例各 17 个中文结构化历史问题通过；这些问题偏向既往史、用药和暴露，不能替代 37 个 canonical slot 的逐问题 UI/API 最小披露矩阵。
- `test-bilingual-conflict-quarantine.ts` 确认 18 条冲突事实保持隔离且没有改变审核状态；该结果只证明技术 quarantine，HEM-P0-023 仍需具名专家裁决。
- 底层双语 slot 中部分文本超过患者答复长度限制；第 3 轮已从实际 `server/patientSession.js`/API 链路验证压缩行为，结论见下节，不再用底层数据单独推断。
- 所有本轮英文/中文答复仍为规则或 fixture。协议通过不等于自然度、纠错能力、长对话一致性或医学正确性通过。

## 2026-07-14 实际规则链路双语审查

- 覆盖 42 例、37 canonical slot、中英文各 2 条人工固定问法，合计 6,216 个路由探针，并逐条重放一次；另有 84 次 session 初始化、168 次明确诊断/报告边界和 13 项公开 handler 烟测。测试未自动生成同义句，避免把随机生成质量混入 matcher 结论。
- 英文开场在 42/42 session 中仍为中文，公开页面四 viewport 均可见，登记 HEM-P1-029。这是语言纯度失败，不依赖医学真值。
- 378 个问法实例因路由缺失、同义改写缺口或“既往史”被诊断/报告边界误拦截，登记 HEM-P1-030；另有 252 个疼痛问法过匹配，登记 HEM-P1-031。
- 191 个唯一 case-slot-language 单元虽有非空、非 unknown 来源，但规则层返回通用“不清楚”，共 365 个问法实例，登记 HEM-P1-032。判断只比较是否丢失为 unknown，不比较来源事实正确性。
- P004 血块与 P005/P006 时相回答含“未主动诉/需追问”等教师元语言。公开 API 直接返回，前端改成泛化澄清句却仍保留已匹配 slot，登记 HEM-P1-033。该结论只判断患者口吻/隐藏提示，不裁决血块或时相本身。
- 18 条 HEM-P0-023 冲突的直接问法继续稳定返回 pending medical review；但 pain 过匹配使其他特异疼痛问题被连带隔离。QA 不比较冲突双方的医学极性，也不解除 quarantine。
- 所有本轮规则/API/UI样本均为 `local-simulation` 或 rule fallback，`providerCalls=0`。因此真实 DeepSeek 的自然度、错误总结纠正、20 轮语义一致性和性能仍保持 Preview 外部阻塞；本轮失败也不能外推为真实 provider 文风结论。

## 2026-07-14 Production `96fcf80` 复核

- 42 例双语矩阵和 17 项公开 handler 烟测改用有效签名 attempt/session 后连续重跑，两次仍得到同一 7 个代表失败及 1,127 个矩阵失败实例；因此 HEM-P1-029–033 不是缺失训练状态造成的假阳性。
- 有效英文 attempt/session 的开场在四 viewport 仍含 CJK；该语言纯度失败与中文→英文切换产生的 401（HEM-P1-034）分开测试，避免授权失败掩盖内容失败。
- P004 教师元语言在有效 session capability 下仍能进入公开 API，四 viewport 复现；结论仍只判断患者口吻与隐藏提示，不裁决血块事实。
- 新的 19 项能力安全矩阵全部通过，但它只证明访问边界、过期和幂等合同；`providerCalls=0`，不能用来通过真实 AI 自然度、错误总结纠正、20 轮语义一致性或医学正确性。
- Production `52c2432` 相对 `96fcf80` 没有 Patient Session、matcher、UI 或医学数据变化；仅 scanner/审计更新不构成自然语言缺陷修复证据。

## 2026-07-14 Production `ff1a932` 复核

- 42/42 英文 session 开场均不再含 CJK，四固定 viewport 英文页面 4/4 `PASS_EMULATION`；HEM-P1-029 关闭为 `RESOLVED_LOCAL_QA`。这只证明确定性本地开场语言纯度，不代表真实 provider 英文自然度。
- 原 6 个教师元语言实例降为 0，P004 四 viewport 4/4 通过；HEM-P1-033 关闭为 `RESOLVED_LOCAL_QA`。161 个不安全确定性来源被明确 fail-closed，仍为 `BLOCKED_SOURCE_REVISION`，QA 未裁决其医学事实。
- 矩阵仍有 1,079 个失败实例/117 组，对应 HEM-P1-030/031/032 的路由、过匹配和 unknown 压缩问题；不得因 029/033 修复而把全 42 例自然语言链路记为通过。
- 中文→英文、英文→中文、快速切换和刷新后切换的工程回归均未再出现 401 / `invalid_attempt_token`；非法、过期和跨病例 session 仍被拒绝。该结果是会话边界通过，不是自然语言或医学通过。
- 本轮 `providerCalls=0`。真实 DeepSeek 的中英文自然度、故意错误总结纠正、20 轮语义一致性、来源日志和性能仍受 Preview 权限/环境阻塞。

## 2026-07-17 Production `8e7d148` 复核

- 42 例、37 canonical slot、中英文双改写的当前矩阵为 6,216/6,216 路由与 6,216/6,216 重放通过，168 个诊断/报告边界通过；HEM-P1-030/031/032 的工程信号均清零。公开 adapter 17/17，`providerCalls=0`。
- 295 次不安全 source-cell 观测继续返回明确 `unsafe_deterministic_answer` 且空 facts/slots，对应既有 161 个来源修订项；P001 肿瘤史/泌尿操作代表项被 QA 强制断言为 fail-closed。该结果是安全隔离通过，不是医学语义或患者自然度通过。
- 18 条双语冲突的直接隔离为 144/144，没有旧 pain 过匹配导致的额外隔离；HEM-P0-023 仍由具名专家裁决。
- 本地 fixture 的英文 20 轮、手动上翻、错误恢复与日志幂等通过，只证明 UI/协议稳定性。当前 Production 的真实 Preview 长对话因 QA 输出安全事件标记 `SECURITY_BLOCKED`；先前 `3fe409f` 的中/英文各 1 个 live_ai 回合不作为当前基线自然语言验收。

## 2026-07-19 Production `3a16f931` 复核

- 受保护 Vercel Preview 精确匹配当前 SHA；两个零重试稳定性批次累计中文 10/10、英文 10/10，全部 `generationSource=live_ai`、provider=`deepseek`、`isFallback=false` 且 history-log=200。该结论证明当前中英文真实 provider 闭环与日志稳定，不等同于逐条医学或文风审批。
- P001 中文单 session 20 轮采用 20 种“何时开始”自然改写，20/20 live AI、20 个 agent request、1 个 session、0 重初始化；刷新前持久化 41 条消息，刷新后全部恢复。测试只保存来源、状态和耗时聚合，不保存完整问答，因此本轮不对20条回答逐字自然度或医学一致性作超出证据的通过结论。
- 第二批 P001–P005 中文完整回答 P95 `1707ms`、英文 `1753ms`；UI 请求发出 P95 均 `42ms`。后续 P001 中文5个live AI样本补得点击至完整患者回答DOM首现P50 `1315.9ms`、P95 `1527.4ms`。服务端`firsttoken`仍只作服务端计时；非流式provider真正首Token明确未测量，HEM-P1-021继续`BLOCKED_MEASUREMENT（DOM子项已测）`。
- P001中文错误总结探针中，患者对“今天首次且一直没有反复”的矛盾复述作出纠正；两轮均live AI，未检测到教师元语言或最终诊断泄露。证据仅保留一致性/泄露布尔值，不保存或逐字引用回答，因此不据此批准医学表达。
- P001英文等价探针也能纠正“today/never before”，且不串中文、不泄露教师元语言或最终诊断。新session中无指代对象的“other part”问题得到自然澄清，未检测到完整病史倾倒；原始回答同样不保存，结论只覆盖这一最小问法。
- 代表类别开放式主诉已扩展到10例×中英文：肿瘤、抗凝、感染、结石、前列腺、肾小球、遗传/儿童线索、女性污染、外伤及高危镜下血尿共20/20 live AI。语言串线、教师元语言、结构化字段和诊断标题启发式均0；因未保留回答正文，本轮不宣称逐字自然度或医学准确性已获人工批准。
- P019/P023/P032/P038/P042的病程时长双改写中，7对live AI均保持同一时长；P032中英文与P042中文进入safety boundary，只验证两次来源一致，不评判回答是否应披露一周/一月。原始回答不保存，结论不越过医学治理。
- Patient Session v2 明确接受 711 个 governed unknown、18 个 unsafe governed unknown 与 144/144 冲突隔离；唯一工程失败是英文 `Have you had a urinary procedure?` 在 42/42 例匹配 `triggers` 而非 `PAST_URINARY_PROCEDURE`，HEM-P1-030 重新标记 `REGRESSED_LOCAL_QA`。这是路由优先级缺陷，不需要 QA 裁决医学真值。
- 中英切换、刷新后切换、快速往返与非法/过期/跨病例 session 拒绝继续通过；HEM-P0-001、HEM-P0-023、来源修订和具名审批保持阻塞。患者主诉文案专项分支未合入、未测试、未修改。
- Production `657ba5d` 仅校正验收文档，与上述应用树代码等价；其状态索引没有消除英文 `urinary procedure` 的42/42路由错配。自然语言/路由结论继续以独立QA v2矩阵为准，等待运行时代码修复后复测。
- P023同session中文→英文→中文的三次病程提问均保持6小时事实；两次中文回答为DeepSeek `live_ai`，英文回答稳定进入`safety_boundary`且明确标记fallback。该结果只证明跨语言事实连续性和治理来源可审计，不把英文安全边界算作真实AI自然度通过，也不评价其进入边界是否医学合宜。
- P038以开放经过、外伤时间、血尿先后、疼痛部位和重复时间确认组成中英文各5轮上下文链。8/10为DeepSeek `live_ai`，中文两次时间问法进入`safety_boundary`；中英文重复时间均一致，未见语言串线、教师/结构化元数据或最终诊断启发式。只据此确认本最小上下文链稳定，不批准逐字表述或医学内容。
- P037中英文刷新前各2轮来源合同正常（最终批3次live AI、1次safety boundary），但刷新后首问均因HEM-P1-045返回401且无回答。因此刷新后的事实/自然语言连续性明确为未评价，不把空回答算作语言串线或医学失败；修复会话能力恢复后再执行原重复事实断言。
- P001中英文内容型Prompt注入/写代码请求均进入`safety_boundary`，未调用provider计时，也未检测到内部字段、教师/评分元语言、最终诊断、可执行代码或语言串线。这里只确认两条最小恶意问法的安全响应，不扩张为全部越狱变体或真实限流通过。
- 数据Agent报告卡中英文均直接显示`final/not_available/not_performed`内部枚举，中文页面还出现英文snake_case，登记HEM-P1-047。四固定viewport按双语分配4/4复现；该结论是学生可见状态文案/内部元数据边界失败，不评价报告医学内容，也不把非医学fixture当作真实AI自然语言样本。
- 英文数据Agent目录与报告内容新增HEM-P1-048：42例英文handler合同中42/42病例、257/257结果含学生可见中文；P008英文页面四viewport均有44个中文控件标签且真实本地报告卡含中文。该结果证明语言串线，不批准中文医学值，也不由QA生成英文医学译文；修复后的英文内容仍需双语医学审核。

## 2026-07-23 `70ea9b3` 自然问法与 Preview 复核

- 用户指定 10 类问法按中文原句与英文工程等价问法覆盖 42 例，共 840 场景。canonical 场景完整命中率 75.00%（630/840），intent 命中率 79.41%（1,134/1,428）；已知事实错误 unknown 0.44%（4/914），可评价极性错误 0%（0/439）。
- missing/隔离行为保持 fail closed：预期 unknown 436/436，医学冲突 42/42 场景、13 个唯一冲突项全部隔离。QA 没有裁决 P019/P020、needs_review 或冲突事实，也没有把不确定回答当作 live_ai 成功。
- 双语工程等义仅 247/420，额外病史 slot 42/840。缺口集中于英文全程/初始/终末问法、中文选择式时相、英文终末否定、英文尿频尿急尿痛复合，以及英文腰痛/发热/血块额外扩张通用 pain；登记 HEM-P1-050。
- 精确 Preview 的 P001 英文错误总结与无指代澄清在全套和隔离复跑均落入 `rule_fallback`。P038 双语五轮中 4/10 为 rule fallback，P037 刷新双语中英文首个上下文问法 1/8 为 rule fallback；请求与 history 均完整，说明是 source 路由而非会话丢失，登记 HEM-P1-051。
- 核心低歧义样本仍通过：中文 5/5、英文 5/5 为 DeepSeek `live_ai` 且 fallback 0；连续 20/20 为 live_ai。该成功率不能覆盖 HEM-P1-050/051 的自然问法缺口，安全边界与规则 fallback 均不冒充真实 AI。
- 报告只保留 case ID、intent、布尔值和聚合计数；完整问题/回答、医学值、token、session 和 Preview 凭据不落盘。进入教师审阅前应先修复两个工程 P1；医学内容最终仍需具名专家审核。

## 2026-07-23 `c4ac9b5` 自然问法与 Preview 复核

- 42例×10问法×双语840/840场景完整命中，canonical intent 1,428/1,428；3,150问扩展矩阵同为3,150/3,150。已知事实错误unknown 0/914、可评价极性错误0/598，正确unknown 436/436，冲突隔离42/42（13个唯一项），双语等义420/420，额外病史0。
- 指定真实问法覆盖“小便痛不痛/排尿时疼吗/没有尿痛吧/小便全程红/从头到尾红/初始终末全程选择/尿频尿急尿痛/腰痛发热血块”；特异dysuria、flank pain不再扩张generic pain，独立通用pain仍可回答。HEM-P1-050更新为 `RESOLVED_LOCAL_QA`。
- 精确Preview中P001英文纠错与澄清3/3、P037英文连续追问2/2、P038英文连续追问2/2均为DeepSeek `live_ai`、非fallback，history7/7。HEM-P1-051更新为 `RESOLVED_PREVIEW`；provider成功没有被后续rule fallback覆盖。
- 低歧义稳定样本中文5/5、英文5/5和单session连续20/20均live_ai，fallback为0。完整回答不落盘；这些工程/来源结果仍不裁决医学内容，不解除HEM-P0-001/023、P019/P020或来源修订状态。

## 2026-07-24 `c4ac9b5` 刷新连续性与路由边界

- P037中英文刷新链共8/8为DeepSeek live_ai，刷新后history完整、病程时长前后2/2一致，未见语言串线、教师元语言、结构字段或最终诊断泄露；HEM-P1-045关闭为`RESOLVED_PREVIEW`。完整回答仍由安全wrapper扫描后删除。
- HEM-P1-030不因真实AI链通过而关闭：本地确定性37槽位矩阵中英文泌尿操作史问法仍在42/42病例命中`triggers`，公开handler最小合法请求也2/2返回同一错误slot和rule fallback。该判断仅评价工程路由，不裁决病例是否存在泌尿操作史。
- HEM-P1-050的840/840自然问法门禁与HEM-P1-030的37槽位门禁覆盖集合不同；前者通过不能替代后者单一遗漏问法失败。修复后需同时保持generic pain、unknown、冲突隔离和最小披露合同。

## 2026-07-24 `c4ac9b5` P001–P042 开放式主诉抽样

- P006–P012中文自然开放主诉三轮累计21/21由DeepSeek `live_ai`回答；英文语义等价自然问法21/21在provider前被`classifier_disabled`路径接管为rule fallback。该结果是问法覆盖/路由质量失败，不是医学事实裁决。
- 本地只读matcher比较显示，自然英文句未命中slot，而canonical控制句命中`chief_complaint`。因此HEM-P1-053至少包含一个可直接修复的英文alias/canonical覆盖缺口。
- canonical控制问法在7例均进入DeepSeek，但首答未被接受并触发重写，最终7/7为`safety_boundary`。QA只保留`responseAccepted=false`与`rewriteTriggered=true`等布尔诊断，不保留回答正文；无法仅凭安全聚合进一步判断是长度、禁词还是其他患者输出规则，修复时需在不记录患者正文的前提下增加安全原因枚举。
- 两条失败路径均未出现中文串线、教师元语言、结构化payload或最终诊断启发式泄露。最终患者措辞是否自然、是否完全忠实于病例仍需具名教师/医学专家审阅，本轮不据工程source结果批准医学内容。
- 同一问法扩展到全42例后，自然中文42/42 live_ai、自然英文42/42 classifier-disabled rule fallback；因此alias/canonical覆盖缺口是全病例问法级失败，不是单个病例事实问题。
- 英文canonical控制的全量分布为31/42 live_ai、11/42 safety boundary。过滤失败精确为P001–P003、P005–P012；其余31例证明canonical问法与DeepSeek链可正常工作，应作为修复非回归组。P019/P020只计source与安全边界，不对其待审核医学内容作通过判断。

## 2026-07-24 `c4ac9b5` P003 七阶段抽样边界

- 本轮P003真实Preview七阶段只使用QA流程占位输入验证阶段推进、幂等、评分与刷新，不向Patient Agent提出医学问题，也不保存诊断或回答正文。
- 因此本轮不新增自然语言或医学质量通过结论；HEM-P1-050已关闭的确定性门禁、HEM-P1-053开放的英文主诉失败，以及HEM-P0-001/023和来源修订阻塞均保持不变。

## 2026-07-24 `c4ac9b5` 跨层复合详细病史

- 单独canonical问法通过不能代表自然复合问法完整。42例双语矩阵显示，症状与既往史/用药/暴露混问时跨层场景613/618失败；典型表现是只回答血尿起病而忽略结石史，只回答尿色而忽略吸烟/职业/家族史，或因canonical事实不确定而把同句确定的过敏/既往史一并丢弃。
- 中文“有没有发热寒战，以前得过尿路感染或肿瘤吗？”在42/42本地及P001–P007 Preview 7/7被当作诊断请求。该问法明确有“以前得过”历史语境，属于工程边界误判，不需要QA判断患者是否真正有肿瘤史。
- 56个包含双语医学冲突的场景56/56继续给出受控不确定并清空收集槽位；这证明HEM-P1-054不是要求放宽冲突隔离，而是要求在合并所有被问子句后统一执行隔离。
- Preview正式35个操作中7次live_ai也仍只有canonical前半句的公开匹配槽位，说明provider自然措辞不能补偿允许事实集合不完整。完整回答不落盘，最终语言自然度仍需教师审阅。

## 2026-07-24 `c4ac9b5` Data Agent阶段权限边界

- 本轮只使用规范医嘱ID/审核显示名和脱敏阶段输入验证结果可达性，没有向Patient Agent提问，也不保存或评价医学结果正文。
- HEM-P1-055与HEM-P2-056均为状态机/渲染工程问题，不新增自然语言质量通过或失败；HEM-P1-053/054、HEM-P0-001/023及来源审核状态保持不变。

## 2026-07-25 `7781586` 病史权威与Preview语言

- P026降糖药、P027别嘌醇、P029抗凝/抗血小板、P039止痛药在本地及真实Preview中英共16条检查均保持权威语义；P029复合问法由安全规则完整回答，不计为live_ai。
- canonical回归为1,428/1,428，自然问法840/840、问法矩阵3,150/3,150、复合问句786/786；错误unknown和极性错误均为0。未审核饮酒/吸烟及“患者未留意”事实保持自然不确定。
- HEM-P1-057的文本表面仍为自然不确定，但provider成功路径返回了可收集匹配元数据；自然措辞不能替代治理状态，故判定P1。
- HEM-P1-058中P037英文6/6均未表达“1 day ago/yesterday”等等价病程，而中文及本地规则正确。完整Preview回答未落盘；缺陷只保存布尔/计数和来源状态。
- 18项双语医学冲突、151项source标记冲突和14项新增阻塞未被解释为工程语言通过；最终自然度仍需在P1修复后由教师人工审阅。

## 2026-07-25 `7781586` 第3–6阶段返回批

- 本轮未向Patient Agent新增问法，也未保存或评价患者回答；HEM-P1-057/058及既有自然语言缺陷状态不变。
- HEM-P2-059中的`Physical examination`和`Awaiting reviewed examination translation`是工程安全占位，不是具名审核翻译。重复标题/React key属于UI渲染问题，修复不得将缺失英文来源改写为翻译通过。

## 2026-07-25 `7781586` 会话持久化与隔离批

- 本轮全部输入为QA流程占位文本，没有向Patient Agent提问、没有保存患者回答，也没有新增医学或自然语言质量结论。
- HEM-P1-060的`stale_attempt_token`是会话版本恢复问题；中文/英文均4个固定viewport交错覆盖，但这只证明错误与界面语言无关，不代表双语患者回答通过。
- 跨语言草稿隔离验证的是客户端存储命名空间，不评价翻译自然度。HEM-P1-057/058、HEM-P0-001/023及来源审核状态保持不变。

## 2026-07-25 `7781586` 多标签页与终态恢复批

- 本轮只使用固定QA流程占位文本，没有向Patient Agent发送新问题、没有保存患者回答，也没有新增自然语言或医学正确性结论。
- 多标签页中英文结果一致仅说明HEM-P1-060与界面语言无关；不能作为双语回答质量通过。P037英文病程HEM-P1-058、provider治理HEM-P1-057及全部医学/来源阻塞状态不变。

## 2026-07-25 `7781586` 存储故障与终态指针隔离批

- 本轮没有向Patient Agent发送问题，也没有保存或评价患者回答、病例事实或最终报告正文；不新增自然语言或医学正确性结论。
- HEM-P1-061中的“跨语言”只表示客户端显示了错误语言作用域的终态界面，不评价该界面的翻译或患者回答质量。HEM-P2-062的英文界面中文警告属于通用UI本地化缺陷，不是医学翻译审核。
- P037英文病程HEM-P1-058、provider治理HEM-P1-057、HEM-P0-001/023以及医学和来源阻塞状态均保持不变。

## 2026-07-25 `7781586` 畸形身份与history队列恢复批

- 畸形pointer和attempt存储恢复测试没有调用Patient Agent，也不保存或评价终态报告正文。
- history-log队列门禁用中英文固定QA问题与固定“未留意”夹具回答触发客户端队列；报告仅保留请求/状态/队列计数，不保留问题、回答或request ID。该夹具不代表病例事实、Patient Agent自然度或医学极性通过。
- HEM-P1-061/063均为客户端身份与持久化工程问题；HEM-P1-057/058、HEM-P0-001/023、医学及来源审核状态不变。

## 2026-07-26 `7781586` 目录与重新开始批

- 本轮没有调用Patient Agent、没有提出医学问题，也没有保存或评价患者回答、病例事实或评分正文。
- 病例目录仍显示既有患者化公开主诉，但本轮只检查目录可用性和进度标签真实性，不新增主诉自然度结论。
- HEM-P1-064/066和HEM-P2-065均为客户端存储/导航工程问题；HEM-P1-057/058、HEM-P0-001/023以及医学/来源阻塞保持不变。
## 2026-07-26 `9b7fcd0` 第 22 轮存储提示与语言切换

- P001 在语言偏好写入失败时，English 操作 4/4 没有切换 React 界面语言，页面正文与 `html lang` 均继续为中文；这不是医学翻译裁决，归入 HEM-P1-064 的界面语言状态原子性。
- 英文损坏缓存恢复场景 2/2 显示中文恢复提示，归入既有 HEM-P2-062；自动保存失败提示本身的中英文文案可识别，但后续成功保存后旧提示未清除。
- 本轮未运行 Patient Agent 问答、自然问法矩阵或医学事实一致性检查；没有新增错误 unknown、极性或医学语言结论。

## 2026-07-26 `9b7fcd0` 第 23 轮 capability 恢复

- 本轮只使用固定QA阶段草稿与训练状态，不调用Patient Agent、不保存问答或病例事实，也不新增canonical、错误unknown、极性、复合问句或双语医学等义结论。
- 中文/英文结果一致仅说明HEM-P1-060与界面语言无关；不能作为患者语言质量通过。HEM-P1-057/058、HEM-P0-001/023及医学/来源阻塞状态保持不变。

## 2026-07-26 `9b7fcd0` 第 24 轮UI P2

- 本轮没有调用Patient Agent、提出医学问题或评价患者回答；不新增canonical、unknown、极性或双语等义结论。
- HEM-P2-059中的`Physical examination`与`Awaiting reviewed examination translation`继续作为安全占位，不代表23个英文来源名称已审核。重复key是纯渲染标识缺陷，修复不得用未审核翻译替换安全文案。

## 2026-07-26 `9b7fcd0` 第 25 轮复合病史与治理投影

- 42例双语复合病史本地矩阵双跑均为786/786：618个跨层子句、56个医学冲突隔离及42个既往肿瘤边界全部满足工程合同；本轮不重新统计840自然问法、1,428 canonical或3,150大矩阵。
- Preview P001–P007中文35问×2轮没有漏掉合法可收集子句，25/35每轮由安全复合规则处理、10/35为live_ai。回答正文未保存，故只评价收集元数据合同，不给出教师自然度通过结论。
- 10个live_ai组的患者文本未发现教师/结构字段泄露，但响应元数据包含待审核槽位；“文字仍自然”不能替代`collectableSlotIds`治理。HEM-P1-057继续OPEN，修复不得把待审核事实改成确定性答案。
- HEM-P0-001/023、32项逐字段医学阻塞、23个英文来源名称和其他来源修订状态均不变；本轮没有新增错误unknown或极性错误结论，也没有医学批准。

## 2026-07-26 `9b7fcd0` 第 26 轮P037英文一天病程

- P037英文开放式主诉在12个全新live_ai会话中仅3/12自然表达`yesterday`，9/12遗漏权威一天病程；没有回答错误的其他时长。该失败是关键信息遗漏，不是措辞偏好。
- 同一Preview的显式“多久以前发现”控制6/6表达一天，说明现有时长事实和英文canonical投影可用；开放式主诉的provider输出保真仍不稳定。
- 完整回答未落盘，因此不对整体自然度、语气或教师体验作通过结论。修复必须保留允许答案中的时间语义，不能通过硬编码整句、修改医学事实或改成rule fallback。
- 本轮没有新增unknown或极性错误统计；HEM-P0-001/023、HEM-P1-057及医学/来源阻塞状态不变。

## 2026-07-26 `9b7fcd0` 第 27 轮英文泌尿操作史路由

- `Have you had a urinary procedure?`在42/42病例均命中`PAST_URINARY_PROCEDURE`治理意图，未再被`triggers`吸收；双跑共84次缺陷探针均无路由漂移。
- 2例已有可收集事实按预期公开slot，40例因审核/观察状态不可收集而自然受控unknown。该40例是正确unknown，不计为错误unknown，也不代表医学真值已获批准。
- 完整矩阵每轮另保持3,020个治理unknown、144/144冲突隔离和0 provider调用；本轮只验证确定性路由/公开投影，不评价live_ai自然度或双语医学含义，不改变HEM-P1-057/058。

## 2026-07-26 `9b7fcd0` 第 28 轮自然问法与极性

- 指定10类中英文问法双跑均840/840场景、1,428/1,428 intent；覆盖尿痛否定、全程/起止/选择/终末否定、尿频尿急尿痛复合及腰痛发热血块复合。
- 当前已知事实错误unknown为0/838、极性错误0/578，512/512真实unknown保持不确定；42/42冲突场景继续隔离，420/420双语工程等义，未问病史额外泄漏0。
- 3,150扩展问法双跑同样零错误unknown和零极性错误；特异dysuria/flank pain不扩张generic pain。上述是确定性工程合同，不替代live_ai措辞、教师体验或具名医学审阅。

## 2026-07-26 `9b7fcd0` 第 29 轮Data Agent语言边界

- 英文公开目录、结构化结果和代表报告卡不再显示中文源文本；预期“中文”语言切换按钮不属于医学内容串线，其他英文控件CJK信号为0。
- 23个未审核英文名称继续使用安全占位，28项缺元数据继续显示待审核；该文案通过只证明fail-closed，不批准具体译名、单位、参考范围或异常含义。
- 本轮没有Patient Agent问答或live_ai自然度结论，HEM-P1-057/058及HEM-P0-001/023状态不变。

## 2026-07-26 `9b7fcd0` 第 30 轮非语言内容回归

- 本轮只验证键盘、焦点、reduced-motion、浏览器语音控制与axe规则，使用脱敏fixture，不生成或保存Patient Agent问题/回答。
- 因此不新增中文/英文自然度、医学语义或live_ai通过结论；既有错误unknown、极性、HEM-P1-057/058及HEM-P0-001/023状态保持。

## 2026-07-26 `9b7fcd0` 第 31 轮双语布局文本边界

- 可访问性矩阵使用42例Production公开患者化开场文本触发布局，但断言只检查页面语言、可见病例ID及axe规则，不评价措辞或医学语义。
- 聚合与原始摘要均不保存开场白、DOM片段、问题或回答；因此不新增Patient Agent自然度、错误unknown、极性或live_ai结论。

## 2026-07-26 `9b7fcd0` 第 32 轮非语言可访问性缺陷

- HEM-P1-067/HEM-P2-068均来自控件语义和键盘焦点，不评价中英文医学措辞；fixture未生成Patient Agent问答，providerCalls 0。
- 可访问名称修复必须保持中英文等义，但QA不得借此修改病例事实、医学文案或审核状态。

## 2026-07-26 `9b7fcd0` 第 33 轮非语言终态隔离

- hydration闪烁探针只使用非医学QA哨兵识别错误终态是否进入DOM；未生成Patient Agent问题/回答，未评价诊断、评分文字或中英文自然度，providerCalls 0。
- 跨语言结果只证明错误语言终态未瞬时呈现，不扩张为Patient Agent双语医学等义、错误unknown或真实AI连续性结论。
