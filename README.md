...
生产 API、健康检查、Vercel/Azure 配置和真实冒烟测试见 [`docs/PRODUCTION_API.md`](docs/PRODUCTION_API.md)。医学专家审核工作簿见 `docs/medical-review/hematuria_case_clinical_review.xlsx`。
...
完整转换连续运行两次的67个受控JSON校验和必须一致。`author_added_for_simulation` 事实必须由医学教师逐项确认。
...
公开站不打包教师答案；`/teacher` 和 `/rct` 只显示安全边界说明。练习attempt使用服务端HMAC签名状态，不再依赖serverless全局Map。正式attempt会校验签名中的mode、caseId和状态，并拒绝未审核病例，但在完成身份验证和持久化审计前仍保持禁用。正式 OSCE/RCT 下一阶段仍需要：
