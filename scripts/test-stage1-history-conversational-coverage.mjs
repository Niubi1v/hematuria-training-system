...
function expectedProjection(caseData, intent, question, language) {
  return projectRoutedPatientFacts(caseData.id, caseData, [{ intent, text: question }], language);
}
...
async function evaluateTurn(caseData, definition, question, language, turnId, sessionId = `stage1-oracle-${turnId}`, conversationHistory = []) {
  ...
  const internalLeakFound = internalLeak.test(String(answer.replyText || ""));
  const blankReply = !String(answer.replyText || "").trim();
...
async function main() {
  ...
  function recordTurn(turn) {
    ...
    for (const [check, metric] of Object.entries(metricByCheck)) {
      ...
      metrics[metric] += 1;
      if (violations.length < 5000) violations.push({ check, ...turn, checks: undefined });
    }
  }
