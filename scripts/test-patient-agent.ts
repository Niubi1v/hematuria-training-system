...
assert(color.matchedSlotIds.includes("HX006"), "color question should match HX006");
...
assert(clot.matchedSlotIds.includes("HX007"), "clot question should match HX007");
assert(/血块|没有注意到|没有明显/.test(clot.replyText), `clot answer should only answer clots: ${clot.replyText}`);
assertNotContains(clot.replyText, ["未主动诉", "未诉", "需追问", "全程血尿", "无痛", "鲜红色", ...forbidden], "clot question");
...
assert(glomerular.matchedSlotIds.includes("HX013"), "foamy urine and edema question should match HX013");
assert(/泡沫|水肿|眼睑|下肢|没有明显/.test(glomerular.replyText), `glomerular clue answer should stay at symptom level: ${glomerular.replyText}`);
...
assert(smoking.matchedSlotIds.includes("HX019"), "smoking question should match HX019");
assert(/烟|吸|不吸|没有/.test(smoking.replyText), `smoking answer should mention smoking only: ${smoking.replyText}`);
