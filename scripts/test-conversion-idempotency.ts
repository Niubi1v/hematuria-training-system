...
try {
  ...
  // Compare generated bytes with committed bytes, independent of a developer's
  // global Windows checkout policy. Without this override, core.autocrlf=true
  // checks committed LF files out as CRLF and every LF-emitting converter looks
  // stale even though Linux CI and the Git objects are identical.
  run("git", ["-c", "core.autocrlf=false", "-c", "core.eol=lf", "worktree", "add", "--detach", worktree, "HEAD"]);
